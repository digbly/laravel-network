  ...
  return (
    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 px-6 py-4 border-t border-slate-100 dark:border-white/[0.06]">
      <p className="text-xs text-slate-500 dark:text-slate-400">
        {t('admin.networkAdmin.pagination.summary', { from: from ?? 0, to: to ?? 0, total })}
      </p>
      ...
      <div className="flex items-center gap-2">
        ...
        >
          {t('admin.networkAdmin.pagination.previous')}
        </Button>

        <span className="text-xs font-medium text-slate-500 dark:text-slate-400 px-1">
          {t('admin.networkAdmin.pagination.page', { current: currentPage, total: lastPage })}
        </span>
        ...
        >
          {t('admin.networkAdmin.pagination.next')}
        </Button>
      </div>
    </div>
  );
